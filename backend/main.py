...
def summarize_with_openai(text: str, bullets: int = 5):
    if not OPENAI_KEY:
...
async def upload_pdf(file: UploadFile = File(...)):
    try:
        print("DEBUG: Received upload:", file.filename)
        if file.content_type != "application/pdf":
            raise HTTPException(400, "Only PDF files allowed.")

        file_id = str(uuid.uuid4())
        save_path = os.path.join(UPLOAD_DIR, f"{file_id}.pdf")
        print("DEBUG: Saving file to", save_path)
        ...
        print(f"DEBUG: Total chunks to embed: {len(chunks)}")
        ...
        print("DEBUG: Adding embeddings to Chroma DB")
        collection.add(documents=texts, embeddings=embeddings.tolist(), metadatas=metadatas, ids=ids)

        sample_text = "\n\n".join(texts[:3])
        print("DEBUG: Sending text to OpenAI for summary")
        summary = summarize_with_openai(sample_text, bullets=5)
        print("DEBUG: OpenAI summary received")
...
async def chat(file_id: str = Form(...), query: str = Form(...), top_k: int = Form(3)):
    ...
    if not OPENAI_KEY:
        llm_answer = "⚠️ OPENAI_API_KEY missing."
    else:
        resp = openai_client.chat.completions.create(
            model=LLM_MODEL,
            messages=[{"role": "user", "content": prompt}],
